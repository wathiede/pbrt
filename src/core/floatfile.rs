...
mod tests {
    ...
    fn good() -> Result<(), Error> {
        ...
        assert_eq!(floats, vec![1., 2.0, 0.5, 10.0]);
        Ok(())
    }
    ...
    fn double_fail_f32() -> Result<(), Error> {
        let floats = read_float_file("src/core/testdata/big.floats")?;
        assert_eq!(floats, vec![Float::INFINITY]);
        Ok(())
    }

    #[test]
    #[cfg(feature = "float-as-double")]
    fn double_succeed_f64() -> Result<(), Error> {
        let floats = read_float_file("src/core/testdata/big.floats")?;
        assert_eq!(floats, vec![1.2345678901234568e39]);
        Ok(())
    }
}
